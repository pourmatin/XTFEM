C amux and dnscsr added by Rui, Mar. 2015

      subroutine amux(n, x, y, a, ja, ia)
c----------------------------------------------------------------------c
c
c! amux sparse matrix * vector multiplication
c
c  Modified:
c    07 January 2004
c
...
c
c  Parameters:
c    Input, integer ( kind = 4 ) N, the order of the matrix.
c    Input, real X(N), the vector to be multiplied.
c    Output, real Y(N), the product A * X.
c    Input, real A(*), integer ( kind = 4 ) JA(*), IA(N+1), the matrix in CSR
...
c----------------------------------------------------------------------c
c DNSCSR converts Dense to Compressed Row Sparse format.
c
c  Discussion:
c    This routine converts a densely stored matrix into a row orientied
c    compactly sparse matrix.  It is the reverse of CSRDNS.
c    This routine does not check whether an element is small.  It considers 
c    that A(I,J) is zero only if it is exactly equal to zero.
c
c  Modified:
c    07 January 2004
c
...
c
c  Parameters:
c    Input, integer ( kind = 4 ) NROW, the row dimension of the matrix.
c    Input, integer ( kind = 4 ) NCOL, the column dimension of the matrix.
c    Input, integer ( kind = 4 ) NZMAX, the maximum number of nonzero elements 
c    allowed.  This should be set to be the lengths of the arrays A and JA.
c    Input, real DNS(NDNS,NCOL), an NROW by NCOL dense matrix.
c    Input, integer ( kind = 4 ) NDNS, the first dimension of DNS, which must be
c    at least NROW.
c    Output, real A(*), integer ( kind = 4 ) JA(*), IA(NROW+1), the matrix in CSR
c    Compressed Sparse Row format.
c    Output, integer ( kind = 4 ) IERR, error indicator.
c    0 means normal return;
c    I, means that the the code stopped while processing row I, because
c       there was no space left in A and JA, as defined by NZMAX.
c
      implicit none
...
c im    == size of krylov subspace:  should not exceed 500 in this     *
