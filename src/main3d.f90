...
! Developed by Hossein Pourmatin @ UT Dallas, Richardson, TX, Sep., 2017
! E-mail: mhp160030@utdallas.edu
...
! Shared Parameters
! Material
REAL(KIND=REKIND) :: ym0, nu0, rho, kappa, cp
! Geometery
REAL(KIND=REKIND) :: le, area
INTEGER :: ne, nn, nnsd, nnte, nquads, nquadt, nnse, ngp, ngpe, n1, n2
...
REAL(KIND=REKIND), DIMENSION(:,:), ALLOCATABLE :: dns_m, ns_m, dnxyz_m
! Spatial matrices
...
INTEGER :: im_m, maxits_m, iout, iout2, ierr
REAL(KIND=REKIND) :: eps_m
! Time-loop
REAL(KIND=REKIND) :: angvel_t, dmax, wsmax
REAL(KIND=REKIND), DIMENSION(6) :: fextc_m
...
! Boundary conditions
REAL(KIND=REKIND) :: ntele_t, ftotal_t, fnode_t, T0
...
! Mesh
ndofn_m   = 3        ! Number of spatial degrees of freedom per node for the Mechanical part
ndofn_t   = 1          ! Number of spatial degrees of freedom per node for the Temperature part
...
angvel_m  = 2._REKIND*pi*freq
! Applid heat flux
q0      = 0._REKIND           ! uniformly distributed heat flux W/mm^2
freq_t    = .02_REKIND      ! Frequency, in Hz
angvel_t  = 2._REKIND*pi*freq
area    = 10._REKIND        ! Traction area mm^2
T0      = 10._REKIND         ! Boundary temprature
! Linear system solver
im_m      = 10                      ! GMRES: size of Krylov subspace
eps_m     = 1.d-8               ! GMRES: tolerance for stopping criterion
maxits_m  = 50                   ! GMRES: maximum number of iterations allowed
iout    = 40                          ! GMRES: solution info output file number
! Linear system solver
im_t      = 10000                 ! GMRES: size of Krylov subspace
eps_t     = 1.d-10              ! GMRES: tolerance for stopping criterion
maxits_t  = 50000              ! GMRES: maximum number of iterations allowed
! Post-processing
...
IF (MPI_ID .EQ. ROOT) THEN
    ...
    WRITE(*,100) '* Number of Mechanical space-time DOFs ', ndofst_m
    WRITE(*,100) '* Number of Temperature space-time DOFs ', ndofst_t
    ! Open data file
...
ENDIF
ALLOCATE(dns_m(ndofn_m*ngpe, nnse), dnxyz_m(ndofn_m**2*ngpe, ndofn_m*nnse), &
& wqs(nquads), ns_m(ndofn_m*ngpe, ndofn_m*nnse))
CALL setshapefunc_m(ndofn_m, nnsd, nnse, nquads, ngpe, wqs, ns_m, dns_m, dnxyz_m)
ALLOCATE(dns_t(ngpe, nnse, 3), ns_t(ngpe, nnse))
CALL setshapefunc_t(nnsd, nnse, nquads, ngpe, wqs, ns_t, dns_t)

!-------------------------------------------------------------------------------------------------------------------------------------------------------
! Formulate spatial matrices (spatial integration) for the Mechanical part
!-------------------------------------------------------------------------------------------------------------------------------------------------------
knz_m = mpi_ne*(nnse*ndofn_m)**2
mnz_m = knz_m
! Initial the Material property
ALLOCATE(tempst(ndofst_t), ym(mpi_ne*8), nu(mpi_ne*8), stiff(6,6,mpi_ne*8))
ym = ym0
nu = nu0
ALLOCATE(Nx(nnse, mpi_ngp), Ny(nnse, mpi_ngp), Nz(nnse, mpi_ngp))
ALLOCATE(kit_m(MAX(knz_m,ndof_m+1)), kjt_m(knz_m), kvt_m(knz_m), &
            & mit_m(MAX(mnz_m,ndof_m+1)), mjt_m(mnz_m), mvt_m(mnz_m))
ALLOCATE(ki_m(ndof_m+1), kj_m(knz_m), kv(knz_m), mi_m(ndof_m+1), &
            & mj_m(mnz_m), mv_m(mnz_m))
CALL mpiglbmtx_m(mpi_ne, mpi_ele, ne, nn, ndofn_m, nnse, nquads, ngpe, ncmp, &
            & xconn, xcoord, ym, nu, rho, wqs, ns_m, dns_m, dnxyz_m, sd, stiff, Nx, Ny, &
            & Nz, kit_m(1:knz_m), kjt_m, kvt_m, knz_m, mit_m(1:mnz_m), mjt_m, mvt_m, mnz_m)
...
mv_m = mvt_m(1:mnz_m)
!-------------------------------------------------------------------------------------------------
! Set boundary conditions for Mechanical part
!-------------------------------------------------------------------------------------------------
IF (MPI_ID .EQ. ROOT) THEN
...
fixedbcst0_m = 0
DO ii = 1, nn
    IF (xcoord(ii,3) .EQ. 0._REKIND .OR. xcoord(ii,3) .EQ. le) THEN
        DO jj = 1, nnte
            DO kk = 1, ndofn
                fixedbcst_m( (ii-1)*ndofn_m+(jj-1)*ndof_m+kk ) = 1
            ENDDO
        ENDDO
    ENDIF
ENDDO
...
!-------------------------------------------------------------------------------------------------------------------------------------------------------
! Formulate spatial matrices (spatial integration) for the Temperature part
!-------------------------------------------------------------------------------------------------------------------------------------------------------
knz_t = mpi_ne*(nnse*ndofn_t)**2
mnz_t = knz_t
ALLOCATE(kit_t(MAX(knz_t,ndof_t+1)), kjt_t(knz_t), kvt_t(knz_t), &
            & mit_t(MAX(mnz_t,ndof_t+1)), mjt_t(mnz_t), mvt_t(mnz_t))
CALL mpiglbmtx_t(mpi_ne, mpi_ele, ne, nn, ndofn_t, nnse, nquads, ngpe, &
            & xconn, xcoord, kappa, cp, rho, wqs, ns, dns, kit_t(1:knz_t), kjt_t, &
            & kvt_t, knz_t, mit_t(1:mnz_t), mjt_t, mvt_t, mnz_t)
...
ALLOCATE(ki_t(ndof_t+1), kj_t(knz_t), kv(knz_t), mi_t(ndof_t+1), mj_t(mnz_t), mv_t(mnz_t))
...
mv0 = mv_t
!-------------------------------------------------------------------------------------------------
! Set boundary conditions for Heat distribution
!-------------------------------------------------------------------------------------------------
IF (MPI_ID .EQ. ROOT) THEN
    CALL CPU_TIME(tcpu(2,1))
ENDIF
...
DO ii = 1, ne
    DO jj = 1, nnse
        kk = xconn(ii,jj)
        IF (xcoord(kk, 3) .EQ. le) THEN
            rext_t(kk*3-0) = rext_t(kk*3-0) + fnode_t
        ENDIF
    ENDDO
ENDDO
...
! MUMPS: initialize
!-----------------------------------------
CALL mumps_solver(mumps_par_m, ROOT, MPI_ID, MPI_P, ndof_m, ndofst_m, nnte_m, ki_m, kj_m, kv_m, tcpu, tusr)
CALL mumps_solver(mumps_par_t, ROOT, MPI_ID, MPI_P, ndof_t, ndofst_t, nnte_t, ki_t, kj_t, kv_t, tcpu, tusr)
...
IF (MPI_ID .EQ. ROOT) THEN
    ...
    ENDIF
ENDIF
ALLOCATE(dispst(ndofst_m), fextst_m(ndofst_m), tcoorde(nnte_m), fextst_t(ndofst_t))
...
DO ii = 1, nstep ! Loop over space time slabs
    ...
    IF (itfail .NE. 0) THEN     
        ...
        CALL delelement(nn, mpi_ne, ndofn_m, nnse, ngpe, ncmp, nquads, &
            & xconn(mpi_ele,:), xcoord, dns_m, dnxyz_m, sd, stiff, wqs, nef, &
        ...
        CALL adddiagonal(nnse, ndofn, ndf, ndfail, ki_m, kj_m, kv_m, ndof_m, knz_m)
        IF (ndf.GT.0) THEN
            DO i = 1, ndf
                j = ndfail(i)
                DO k = 1, 2*nnte
                    fixedbcst0_m((j-1)*ndofn_m+(k-1)*ndof_m+1 : j*ndofn_m+(k-1)*ndof_m) &
                        & = [1, 1, 1]
                ENDDO
            ENDDO
        ENDIF
        ...
        fixedbcst0_m = fixedbcst_m
        CALL csrapplybc(mi_m, mj_m, mv_m, ndof_m, mnz_m, fixedbcst_m(1:ndof_m))
        CALL csrapplybc_m(ki_m, kj_m, kv_m, ndof_m, knz_m, fixedbcst_m(1:ndof_m))
    ...
    ENDDO
    ! Calculate temporal component of heat flux vectors
    IF (ii .EQ. 1) THEN
        fextst_t = fInit
    ELSE
        fextst_t = 0._REKIND
    ENDIF
    CALL getfextc(ii, angvel_t, dt, fextc_t)
    kk = 0
    ll = 1 - ndof
    DO jj = 1, nnte
        kk = kk + ndof
    ...
    fextst_m = 0._REKIND
    CALL getfextc(ii, angvel_m, dt, fextc_m)
    kk = 0
    ll = 1 - ndof
    DO jj = 1, nnte
        kk = kk + ndof
        ll = ll + ndof
        ! Space time external force analagous vector
        fextst_m(ll:kk) = fextc_m(jj)*rext_m
    ENDDO
    !-------------------------------------------------------
    ...
    !-------------------------------------------------------------------
    CALL material_update(nnse, nquads, ne, ngpe, mpi_ne, ele, xconn, &
    ...
    CALL mpipgmres(ck_m+cktn_m, cm_m+cmtn_m, 2*nnte, ki_m, kj_m, kv_m, mi_m, mj_m, &
        & mv_m, ndof_m, mumps_par_m, fextst_m, dispst, im_m, eps_m, maxits_m, iout, ierr)
    IF (MPI_ID .EQ. ROOT) THEN
    ...
    IF (MPI_ID .EQ. ROOT) THEN
        DO jj = 1, ndofst_m
            IF (fixedbcst_m(jj) .NE. 0) dispst_m(jj) = 0._REKIND
        ENDDO
    ENDIF
    CALL MPI_Barrier(MPI_COMM_WORLD, MPI_IERR)
    IF (MPI_IERR .NE. 0) THEN
    ...
    CALL kokkos_damage(MPI_ID, job, mpi_ne, nn, ndofn_m, nnse, nnte, ngpe, &
                & ncmp, nip, xconn(mpi_ele,:), Nx, Ny, Nz, angvel_m, &
...
DEALLOCATE(xcoord, xconn, dispst, tempst, ym, nu, fextst_t, fextst_m, tcoorde, wqs)
DEALLOCATE(dns_m, ns_m, dns_t, ns_t, Nx, Ny, Nz, dnxyz_m, stiff)
DEALLOCATE(fixedbcst_m, fixedbcst0_m, rext_m, fixedbcst_t, fixedbcst0_t, rext_t)
...
DEALLOCATE(kit_t, kjt_t, kvt_t, mit_t, mjt_t, mvt_t)
